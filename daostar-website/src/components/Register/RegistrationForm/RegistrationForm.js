...
const RegistrationForm = ({ toggleRegScreen, setRegistrationData }) => {
  ...
  const { ensData, isError, isLoading } = useEnsAddress({
  ...
  function OnRegister() {
    const checkIfENS = daoContractAddress.includes('.')
    if(checkIfENS){
      setDaoContractAddress(ensData)
    }
    ...
    if (daoFramework === "snapshot") {
      ...
      if (!validator.isEthereumAddress(daoContractAddress))
